...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry):
    ...
    state_proxy = await hass.async_add_executor_job(
        lambda: UponorStateProxy(hass, host, store)
    )
    ...
    async def handle_set_variable(call):
        ...
        await hass.data[DOMAIN]["state_proxy"].async_set_variable(var_name, var_value)
...
async def async_update_options(hass: HomeAssistant, entry: ConfigEntry) -> None:
    """Update options."""
    _LOGGER.debug(
        "Update setup entry: %s, data: %s, options: %s",
        entry.entry_id,
        entry.data,
        entry.options,
    )
...
async def async_unload_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    _LOGGER.debug(
        "Unloading setup entry: %s, data: %s, options: %s",
        config_entry.entry_id,
        config_entry.data,
        config_entry.options,
    )
    unload_ok = await hass.config_entries.async_unload_platforms(
        config_entry, [Platform.SWITCH, Platform.CLIMATE]
    )
    return unload_ok
...
class UponorStateProxy:
    ...
    def get_setpoint(self, thermostat):
        ...
        if var in self._data:
            ...
            return math.floor(
                (int(self._data[var]) - self.get_active_setback(thermostat, temp) - 320)
                / 1.8
            ) / 10

    def get_active_setback(self, thermostat, temp):
        if temp == self.get_min_limit(thermostat) or temp == self.get_max_limit(
            thermostat
        ):
            return 0
        ...
        if var_eco_setback in self._data and (
            self.is_eco(thermostat) or self.is_away()
        ):
    ...
    async def async_switch_to_cooling(self):
        ...
        await self._hass.async_add_executor_job(
            self._client.send_data, {"sys_heat_cool_mode": "1"}
        )
    ...
    async def async_switch_to_heating(self):
        ...
        await self._hass.async_add_executor_job(
            self._client.send_data, {"sys_heat_cool_mode": "0"}
        )
    ...
    async def async_set_away(self, is_away):
        ...
        await self._hass.async_add_executor_job(self._client.send_data, {var: data})
        self._data[var] = data
        async_dispatcher_send(self._hass, SIGNAL_UPONOR_STATE_UPDATE)
    ...
    async def async_set_variable(self, var_name, var_value):
        _LOGGER.debug(
            "Called set variable: name: %s, value: %s, data: %s",
            var_name,
            var_value,
            self._data,
        )
